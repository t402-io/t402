...
\section{Cosmos Implementation}
\label{sec:cosmos-implementation}
...
\subsection{Native Token Standard}

Noble hosts native USDT with the \code{uusdt} denomination (micro-USDT, 6 decimals). Transfers use the standard Cosmos \code{bank/MsgSend} message:

\begin{lstlisting}[language=json,caption={Cosmos bank send message}]
{
...
\begin{lstlisting}[language=json,caption={Cosmos exact scheme payload}]
{
  "signedTx": "base64-encoded-signed-transaction",
  "publicKey": "base64-encoded-public-key",
  "accountAddress": "noble1..."
}
\end{lstlisting}
...
\begin{itemize}
    \item Validate secp256k1 signature
    \item Verify sender matches \code{fromAddress}
    \item Check denomination is \code{uusdt}
    \item Validate recipient matches \field{payTo}
    \item Confirm sufficient balance
    \item Verify chain ID matches Noble mainnet (\code{noble-1}) or testnet (\code{grand-1})
\end{itemize}
...
\begin{tabular}{llllll}
\toprule
\textbf{Aspect} & \textbf{Aptos} & \textbf{Tezos} & \textbf{Polkadot} & \textbf{Stacks} & \textbf{Cosmos} \\
\midrule
...
Finality & $\sim$1s & $\sim$30s & $\sim$12s & $\sim$10min & $\sim$6s \\
\bottomrule
\end{tabular}
\end{table}
