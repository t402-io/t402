...
const OUTPUT_TS = path.join("src/stacks/gen", "template.ts");
...
const OUTPUT_PY = path.join(PYTHON_DIR, "stacks_paywall_template.py");
const OUTPUT_GO = path.join(GO_DIR, "stacks_paywall_template.go");
...
/**
 * Builds the Stacks paywall HTML template with bundled JS and CSS.
...
async function build() {
  try {
    ...
    console.log("[Stacks] Build completed successfully!");
...
/**
 * The pre-built Stacks paywall template with inlined CSS and JS
 */
export const STACKS_PAYWALL_TEMPLATE = ${JSON.stringify(html)};
`;

      // Generate Python template file
      const pyContent = `# THIS FILE IS AUTO-GENERATED - DO NOT EDIT
STACKS_PAYWALL_TEMPLATE = ${JSON.stringify(html)}
`;
...
// StacksPaywallTemplate is the pre-built Stacks paywall template with inlined CSS and JS
const StacksPaywallTemplate = ${JSON.stringify(html)}
`;

      fs.writeFileSync(OUTPUT_TS, tsContent);
      console.log(`[Stacks] Generated template.ts (${(html.length / 1024 / 1024).toFixed(2)} MB)`);

      // Write the Python template file
      if (fs.existsSync(PYTHON_DIR)) {
        fs.writeFileSync(OUTPUT_PY, pyContent);
        console.log(
          `[Stacks] Generated Python stacks_paywall_template.py (${(html.length / 1024 / 1024).toFixed(2)} MB)`,
        );
      } else {
        console.warn(`[Stacks] Python directory not found: ${PYTHON_DIR}`);
      }
      ...
      if (fs.existsSync(GO_DIR)) {
        ...
        console.log(
          `[Stacks] Generated Go stacks_paywall_template.go (${(html.length / 1024 / 1024).toFixed(2)} MB)`,
        );
      } else {
        console.warn(`[Stacks] Go directory not found: ${GO_DIR}`);
      }
    } else {
