/**
 * Shared utility functions for data-services (bazaar + explorer).
 */
...
const ADDR_URLS = {
  ...
  "stellar:pubnet": "https://stellar.expert/explorer/public/account/",
};

export function getExplorerUrl(network, txHash) {
  return network && txHash && TX_URLS[network] ? TX_URLS[network] + txHash : null;
}

export function getAddressUrl(network, address) {
  return network && address && ADDR_URLS[network] ? ADDR_URLS[network] + address : null;
}
