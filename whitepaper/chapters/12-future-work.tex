...
% Phase markers
\foreach \x/\label in {0/Q1'25, 3.5/Q3'25, 7/Q1'26, 10.5/Q3'26, 14/Q1'27} {
...
\node[phase=t402green] at (5.25,1.2) {Phase 2};
\node[milestone=t402green] at (3.5,0) {};
\node[milestone=t402green] at (7,0) {};

\node[feature] at (5.25,2.2) {Up-To Scheme};
\node[feature] at (5.25,2.9) {Cosmos + HW Wallet};
\node[feature] at (5.25,3.6) {MCP Parity};
...
% Legend
\node[font=\tiny, gray] at (7,-1) {Completed \textbullet{} Planned};
...
\begin{description}
    \item[Phase 1 -- Foundation (Completed)] Core protocol specification, SDK implementations in TypeScript, Python, Go, and Java, plus the Facilitator service MVP with 10 blockchain families and 44+ networks.

    \item[Phase 2 -- Enhancement (Completed)] Protocol extensions including Up-To scheme (implemented for SVM, TON, TRON, NEAR), hardware wallet support (TypeScript, Go), Cosmos chain support, MCP tool parity across all SDKs, and additional HTTP framework integrations.

    \item[Phase 3 -- Scale (Planned)] Infrastructure scaling with multi-region deployment, new SDK platforms (Rust, Swift), and subscription billing support.

    \item[Phase 4 -- Standardize (Planned)] Industry standardization through IETF, W3C integration, and advanced privacy features using zero-knowledge proofs.
\end{description}
...
The ``up-to'' scheme enables metered, usage-based billing where clients authorize a maximum amount upfront, and providers settle only the actual usage. The up-to scheme is implemented for Solana (SVM), TON, TRON, and NEAR chains. EVM support via Permit2 is planned for a future release. This pattern is essential for:
...
For EVM networks, the up-to scheme leverages EIP-2612~\cite{eip2612} permits with batch settlement:

\begin{lstlisting}[language=solidity, caption={Up-To Settlement Contract (Simplified)}]
contract UpToSettlement {
    ...
    function initSession(address client, uint256 max,
        uint256 deadline, uint8 v, bytes32 r, bytes32 s) external {
        usdt.permit(client, address(this), max, deadline, v, r, s);
        // Create session with max authorization
    }

    function settle(bytes32 id, uint256 actual) external {
        require(actual <= sessions[id].maxAmount);
        usdt.transferFrom(sessions[id].client, msg.sender, actual);
    }
}
\end{lstlisting}
...
Uniswap's Permit2~\cite{permit2} contract provides significant improvements over traditional token approvals:
...
async function createPermit2Payment(amount: bigint, recipient: string) {
  const permit = {
    permitted: { token: USDT, amount },
    spender: FACILITATOR, nonce: await getNextNonce(), deadline
  };
  const signature = await wallet._signTypedData(/*...*/);
  return { scheme: 'permit2', payload: encodeBase64({ permit, signature }) };
}
\end{lstlisting}
...
CAIP-122~\cite{caip122} defines a chain-agnostic authentication standard enabling wallet-based identity:
...
\begin{lstlisting}[language=typescript, caption={SIWx + T402 Middleware (Conceptual)}]
async function authMiddleware(req: Request) {
  // Priority: 1) Session credit, 2) T402 payment, 3) SIWx identity
  if (req.headers.get('Authorization')) return checkCredit(req);
  if (req.headers.get('PAYMENT-SIGNATURE')) return processPayment(req);
  if (req.headers.get('X-SIWx')) return verifySIWx(req);
...
\begin{itemize}
    \item Native async/await support with Tokio
    \item Server middleware for Axum and Actix-web
    \item WebAssembly compilation for browsers and edge computing
    \item Zero-copy parsing for high-performance scenarios
\end{itemize}
...
\begin{itemize}
    \item Swift Concurrency (async/await) for modern iOS development
    \item SwiftUI components for declarative payment UIs
    \item WalletConnect integration for mobile wallet connections
    \item Keychain-based secure key storage
    \item App Clips support for lightweight payment flows
\end{itemize}
...
\begin{itemize}
    \item Kotlin Coroutines for seamless async operations
    \item Jetpack Compose components for Material Design paywalls
    \item Android Keystore integration for secure key management
    \item OkHttp interceptor for automatic payment handling
\end{itemize}
...
\begin{itemize}
    \item Minimal dependencies (libcurl, OpenSSL)
    \item Static memory allocation for constrained devices
    \item FreeRTOS and Arduino support
    \item Header-only option for easy integration
\end{itemize}
...
\begin{itemize}
    \item \textbf{Tiered limits}: Different quotas for anonymous, authenticated, and enterprise clients
    \item \textbf{Adaptive throttling}: Automatic adjustment based on system load
    \item \textbf{DDoS integration}: Cloudflare and similar provider integration
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Defines \header{PAYMENT-SIGNATURE}, \header{PAYMENT-REQUIRED}, and \header{PAYMENT-RESPONSE} headers
...
\begin{enumerate}
    \item \textbf{2026}: Draft submission to IETF HTTP Working Group
    \item \textbf{2026--2027}: Working group adoption and review
    \item \textbf{2027}: Last Call and RFC publication
\end{enumerate}
...
// Use paymentHeader in PAYMENT-SIGNATURE header
\end{lstlisting}
