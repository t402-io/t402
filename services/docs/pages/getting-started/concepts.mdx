...
- **exact**: Pay an exact amount via off-chain signature (EIP-3009 for EVM, SPL delegate for Solana, Jetton for TON, TRC-20 for TRON)
- **exact-direct**: Pay an exact amount via on-chain transfer with tx hash proof (NEAR, Aptos, Tezos, Polkadot, Stacks, Cosmos)
- **upto**: Authorize up to a maximum amount, pay based on actual usage (EIP-2612 for EVM)
- **exact-legacy**: Legacy direct token transfers for EVM chains without EIP-3009 support

See [Payment Schemes](/resources/schemes) for detailed documentation.
...
interface PaymentRequirements {
  ...
  // For 'exact' scheme:
  amount?: string           // Fixed amount (e.g., '1000000' for 1 USDT)
  // For 'upto' scheme:
  maxAmount?: string        // Maximum authorized amount
  minAmount?: string        // Minimum settlement amount
  // Common fields:
