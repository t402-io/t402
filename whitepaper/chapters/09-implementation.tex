...
\midrule
TypeScript & 2.4.0 & npm @t402/* & Express, Hono, Fastify, Next.js & Apache 2.0 \\
Python & 1.10.1 & PyPI & FastAPI, Flask, Django, Starlette & Apache 2.0 \\
Go & 1.9.0 & Go Modules & Gin, Echo, Chi, Fiber & Apache 2.0 \\
Java & 1.10.0 & Maven Central & Servlet, Spring, WebFlux, Micronaut, Quarkus & Apache 2.0 \\
\bottomrule
\end{tabular}
\end{table}
...
The TypeScript SDK is organized as a monorepo with 29 modular packages:
...
\midrule
Core & core, evm-core, extensions & Type definitions, core logic \\
Client & fetch, axios & HTTP clients with auto-payment \\
Server & express, fastify, hono, next & Server middleware \\
AI & mcp, a2a & MCP server, A2A transport \\
Chains & evm, svm, ton, tron, near, aptos, tezos, polkadot, stacks, cosmos & Chain-specific handlers \\
UI & paywall, react, vue & Frontend components \\
WDK & wdk, wdk-gasless, wdk-bridge, wdk-multisig & Tether WDK integration \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{lstlisting}[language=typescript,caption={MCP server with paid tools (conceptual)}]
...
\begin{lstlisting}[language=typescript,caption={Unit testing with mocks (conceptual)}]
import { describe, it, expect, vi } from "vitest";
import { createMockPayment, MockFacilitator } from "@t402/core/testing";
...
\begin{lstlisting}[language=typescript,caption={Integration testing with testnet (conceptual)}]
import { T402TestClient } from "@t402/core/testing";
