/**
 * Bazaar service store — SQLite-backed with in-memory fallback.
 *
 * Accepts a db instance from the caller instead of creating its own.
 * Call initBazaarStore(db) to create table and return the store object.
 */
...
// ── initBazaarStore ──────────────────────────────────────────────────
export function initBazaarStore(db) {
  let useMemory = false;
  ...
  if (db) {
    try {
      ...
      logger.info("bazaar store initialized", { engine: "sqlite" });
    } catch (e) {
      logger.warn("SQLite unavailable for bazaar, using in-memory store", { error: e.message });
      useMemory = true;
    }
  } else {
    useMemory = true;
  ...
  function seedStore() {
    ...
    if (allSeeds.length === 0) {
      logger.info("bazaar store already seeded", { services: store.size() });
      return;
    }
    ...
    logger.info("bazaar store seeded", { services: store.size() });
  }
  ...
  return { store, seedStore, getNextId };
}
