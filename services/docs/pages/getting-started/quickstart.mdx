...
**Testing first?** Use the [Sandbox](https://sandbox.t402.io) to test verify/settle flows with magic test addresses — no API key, no real funds, no wallet needed. Try the [interactive playground](https://sandbox.t402.io/playground).
</Callout>
...
1. Go to [facilitator.t402.io/register](https://facilitator.t402.io/register)
2. Enter your email and project name
3. Copy your `t402_sk_...` API key

Free tier: **1,000 transactions/month**, all 11 chain families, 0% fee. See [pricing tiers](/reference/facilitator-api#authentication) for details.

Your key unlocks the [dashboard](https://facilitator.t402.io/dashboard) with usage charts, key rotation, webhook settings, and IP restrictions.
...
```bash
pip install t402
# or: uv add t402 / poetry add t402
```

Includes all mechanisms, FastAPI/Flask/Django integrations, and HTTPX client wrapper.
...
Includes all 13 mechanisms, HTTP middleware (net/http, Gin, Echo, Chi), and MCP server.
...
Includes Spring Boot auto-configuration, all mechanisms, ERC-4337, and LayerZero bridge.
...
| Language | Version |
|----------|---------|
| TypeScript | Node.js 18+ or Bun 1.0+, TypeScript 5.0+ |
| Python | Python 3.10+ |
| Go | Go 1.24+ |
...
- [Guides](/guides) — Chain integration tutorials and use cases
