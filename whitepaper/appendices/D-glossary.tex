...
\item[CAIP-2]
Chain Agnostic Improvement Proposal 2, a standard format for blockchain network identifiers. Format: \code{namespace:reference} (e.g., \code{eip155:8453} for Base, \code{solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp} for Solana mainnet).
...
\item[PaymentPayload]
JSON structure containing a signed payment authorization sent by clients in the \code{X-PAYMENT} header. Includes payer, signature, and authorization details.
...
\item[Scheme]
Payment logic defining how amounts are calculated and validated. Currently \tprotocol{} supports ``exact'' (amount matches exactly); ``up-to'' (streaming/variable) planned.
...
\item[Up-To Scheme]
Planned \tprotocol{} payment scheme for streaming or variable amounts, where the client authorizes up to a maximum and the server settles the actual consumed amount.
...
\item[Westend]
Polkadot's canary network for testing, with Asset Hub at CAIP-2 \code{polkadot:e143f23803ac50e8f6f8e62695d1ce9e}.
...
\item[X-PAYMENT]
HTTP header containing the base64url-encoded payment payload in \tprotocol{} requests.

\item[X-PAYMENT-REQUIRED]
HTTP header containing base64url-encoded payment requirements in 402 responses.

\item[X-PAYMENT-RESPONSE]
