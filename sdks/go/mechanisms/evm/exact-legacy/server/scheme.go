...
// ExactLegacyEvmScheme implements the SchemeNetworkServer interface for EVM exact-legacy payments.
// Enhances payment requirements with spender, tokenType, and EIP-712 domain info
// needed for the approve + transferFrom pattern.
type ExactLegacyEvmScheme struct {
	moneyParsers []t402.MoneyParser
}

// NewExactLegacyEvmScheme creates a new ExactLegacyEvmScheme
func NewExactLegacyEvmScheme() *ExactLegacyEvmScheme {
...
func (s *ExactLegacyEvmScheme) Scheme() string {
...
func (s *ExactLegacyEvmScheme) RegisterMoneyParser(parser t402.MoneyParser) *ExactLegacyEvmScheme {
...
func (s *ExactLegacyEvmScheme) ParsePrice(price t402.Price, network t402.Network) (t402.AssetAmount, error) {
...
func (s *ExactLegacyEvmScheme) parseMoneyToDecimal(price t402.Price) (float64, error) {
	switch v := price.(type) {
	case string:
...
// defaultMoneyConversion converts decimal amount to the default asset amount
func (s *ExactLegacyEvmScheme) defaultMoneyConversion(amount float64, network t402.Network) (t402.AssetAmount, error) {
	...
	// (e.g., 1500000 for $1.50 USDT is likely already in smallest unit, not $1.5M)
	oneUnit := float64(1)
...
// EnhancePaymentRequirements adds exact-legacy specific fields to payment requirements.
// Adds spender (facilitator address), tokenType ("legacy"), and EIP-712 domain info.
func (s *ExactLegacyEvmScheme) EnhancePaymentRequirements(
	ctx context.Context,
	...
	if _, ok := requirements.Extra["name"]; !ok {
		requirements.Extra["name"] = "T402LegacyTransfer"
	}
	...
	requirements.Extra["tokenType"] = "legacy"

	// Copy spender from supportedKind.Extra (facilitator address)
	if supportedKind.Extra != nil {
		if spender, ok := supportedKind.Extra["spender"].(string); ok {
			requirements.Extra["spender"] = spender
		}
	}
...
func (s *ExactLegacyEvmScheme) GetDisplayAmount(amount string, network string, asset string) (string, error) {
	// Get asset info
	...
	// Add currency symbol
	return "$" + formatted + " USDT", nil
}

// ValidatePaymentRequirements validates that requirements are valid for this scheme
func (s *ExactLegacyEvmScheme) ValidatePaymentRequirements(requirements t402.PaymentRequirements) error {
...
func (s *ExactLegacyEvmScheme) ConvertToTokenAmount(decimalAmount string, network string) (string, error) {
...
func (s *ExactLegacyEvmScheme) ConvertFromTokenAmount(tokenAmount string, network string) (string, error) {
	config, err := evm.GetNetworkConfig(network)
	if err != nil {
		return "", err
	}
...
func (s *ExactLegacyEvmScheme) GetSupportedNetworks() []string {
...
func (s *ExactLegacyEvmScheme) GetSupportedAssets(network string) ([]string, error) {
	config, err := evm.GetNetworkConfig(network)
	if err != nil {
		return nil, err
	}
