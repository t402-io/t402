...
const config: BuildConfig = {
  network: "cosmos",
  entryPoint: "src/cosmos/entry.tsx",
  templateConstName: "COSMOS_PAYWALL_TEMPLATE",
  goConstName: "CosmosPaywallTemplate",
};
...
async function build() {
  try {
    ...
    const genDir = path.join("src/cosmos/gen");
    ...
    console.log("[Cosmos] Browser bundle built successfully!");

    // Build inline HTML template (existing behavior)
    await esbuild.build(options);
    console.log("[Cosmos] Inline HTML build completed!");
