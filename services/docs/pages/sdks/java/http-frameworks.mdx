...
The Java SDK provides payment filters for 5 HTTP frameworks: Jakarta Servlet, Spring MVC, Spring WebFlux, Micronaut, and JAX-RS. Each filter implements the full T402 v2 payment lifecycle with V2-first header detection and V1 fallback.
...
| JAX-RS | `io.t402.jaxrs` | `PaymentFilter` | Synchronous (JAX-RS) |
...
The JAX-RS filter implements `ContainerRequestFilter` and `ContainerResponseFilter` for the full payment lifecycle. Works with any JAX-RS runtime (Jersey, RESTEasy, etc.).

```java
import io.t402.jaxrs.PaymentFilter;
import io.t402.client.HttpFacilitatorClient;
...
All filters share the same constructor pattern:
