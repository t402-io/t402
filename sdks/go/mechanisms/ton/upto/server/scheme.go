// Package server implements the server role for TON upto payments.
package server
...
// UptoTonServer implements the SchemeNetworkServer interface for TON upto payments
type UptoTonServer struct {
	moneyParsers []t402.MoneyParser
}

// NewUptoTonServer creates a new UptoTonServer
func NewUptoTonServer() *UptoTonServer {
	return &UptoTonServer{
		moneyParsers: []t402.MoneyParser{},
	}
}

// Scheme returns the scheme identifier
func (s *UptoTonServer) Scheme() string {
	return upto.SchemeUpto
}

// RegisterMoneyParser registers a custom money parser
func (s *UptoTonServer) RegisterMoneyParser(parser t402.MoneyParser) *UptoTonServer {
	s.moneyParsers = append(s.moneyParsers, parser)
	return s
}

// ParsePrice converts a Price value to an AssetAmount for TON
func (s *UptoTonServer) ParsePrice(price t402.Price, network t402.Network) (t402.AssetAmount, error) {
...
func (s *UptoTonServer) parseMoneyToDecimal(price t402.Price) (float64, error) {
...
func (s *UptoTonServer) defaultMoneyConversion(amount float64, config *ton.NetworkConfig) (t402.AssetAmount, error) {
...
// EnhancePaymentRequirements adds TON-specific upto fields to payment requirements
func (s *UptoTonServer) EnhancePaymentRequirements(
	ctx context.Context,
	...
	// Get network config
	config, err := ton.GetNetworkConfig(string(requirements.Network))
	if err != nil {
	...
	if requirements.Asset == "" {
		requirements.Asset = config.DefaultAsset.MasterAddress
	}
...
func (s *UptoTonServer) SetMaxAmount(requirements *types.PaymentRequirements, maxAmount string) error {
	if requirements.Extra == nil {
		requirements.Extra = make(map[string]interface{})
	}
...
func (s *UptoTonServer) SetMinAmount(requirements *types.PaymentRequirements, minAmount string) error {
	if requirements.Extra == nil {
		requirements.Extra = make(map[string]interface{})
	}
...
func (s *UptoTonServer) SetBillingUnit(requirements *types.PaymentRequirements, unit string, unitPrice string) error {
	if requirements.Extra == nil {
		requirements.Extra = make(map[string]interface{})
	}
...
func (s *UptoTonServer) CalculateSettleAmount(requirements types.PaymentRequirements, units int64) (string, error) {
...
func (s *UptoTonServer) FormatAmount(amount string, decimals int) string {
...
func (s *UptoTonServer) ValidateSettleAmount(requirements types.PaymentRequirements, settleAmount string) error {
...
func (s *UptoTonServer) GetTokenDecimals(network t402.Network, asset string) int {
	...
	if asset == config.DefaultAsset.MasterAddress {
