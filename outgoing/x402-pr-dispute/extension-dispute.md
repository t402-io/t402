...
The `dispute` extension provides cryptographic envelopes for the post-settlement reversibility cycle:

- **SignedDispute**: a payer (or its delegate) signs a complaint against a previously issued receipt, requesting a full or partial refund.
...
Together with the [`offer-receipt`](./extension-offer-and-receipt.md) extension, this completes the four-step chain that gives x402 payments enforceable buyer-side recourse:
...
This is a **Client ↔ Server ↔ Arbiter** extension. The facilitator MAY act as arbiter (`arbiterScheme: "facilitator"`) but is not required.

The signed payloads are **x402 version-agnostic** and work identically for both x402 v1 and v2.

---

## Motivation

The base [`exact`](../schemes/exact/scheme_exact.md) scheme produces irreversible transactions; once settled, neither side can reverse the payment. For three use cases x402 explicitly targets, this is unacceptable:

1. **Non-US merchant chargeback equivalence** — credit-card networks provide chargebacks; x402 needs an equivalent recourse path for jurisdictions where merchants accept this risk.
...
Without this extension, every x402 integrator must invent its own dispute envelope, breaking interoperability.

### Relationship to PRs #2493–2495

PRs #2493 (compliance-receipt), #2494 (refund-receipt + cancellation-receipt), and #2495 (pre-payment-compliance-gate) propose a PSD2-regulatory receipt substrate. The `dispute` extension is **complementary, not duplicative**:

- The PSD2 receipts capture compliance metadata required by regulated payment institutions.
- The `dispute` extension captures the EIP-712-signed envelope that binds a client complaint to a receipt and an arbiter verdict to that complaint, independent of regulatory context.

A client may submit a `dispute` against any `offer-receipt` chain whether or not the merchant is a regulated institution. When both are present, the PSD2 `refund-receipt` can be referenced from `Resolution.refundTransaction` as an `offchain://` URI.
...
Servers MAY declare dispute terms by including the `dispute` extension. This advertises (a) which arbiter the server uses, (b) the dispute window, and (c) the supported dispute reasons.
...
| `arbiterScheme`      | string           | yes      | One of `"facilitator"` (the x402 facilitator acts as arbiter), `"contract"` (a smart-contract arbiter resolves on-chain), `"external"` (a third-party off-chain arbiter), `"none"` (disputes accepted but resolution is informal)                          |
...
Disputes are not transmitted on the payment-request path. They are submitted to a dispute endpoint advertised by the server or facilitator (e.g. `POST /v2/dispute`). The transport-level binding is:
...
| `receiptHash`     | string (hex)    | yes      | EIP-712 hash of the `SignedReceipt` being disputed (from [`offer-receipt`](./extension-offer-and-receipt.md))                                |
...
| `refundTransaction` | string           | no       | On-chain refund transaction hash (present when the resolution triggered an on-chain refund via `auth-capture` or `batch-settlement`); MAY be an off-chain reference (e.g. `"offchain://wire/2026-05-28/INV-123"`) |
...
The `chainId` is hardcoded to `1` for the same reasons explained in [`offer-receipt`](./extension-offer-and-receipt.md) §3.2 (off-chain signing format independent of payment network).
...
The canonical `types` and `primaryType` definitions MUST NOT be included in transmitted x402 messages. Signers MUST use the canonical definitions from this specification when producing EIP-712 signatures. Any change to the canonical `types` or `primaryType` constitutes a breaking change and MUST be accompanied by bumping the EIP-712 domain `version`.

### JWS

JSON Web Signature format for non-EVM contexts. Reserved for future specification; expected to use ES256K and the same payload structure with JCS canonicalization. Implementations MAY reject JWS-format envelopes until this section is normative.
...
The x402 facilitator that processed the payment acts as arbiter. The arbiter address matches the facilitator's signing key. This is the default for self-hosted merchants who delegate trust to the facilitator they already use.
...
The Dispute extension composes with other x402 schemes and extensions as follows:
...
| Dispute + PR #2494 PSD2 `refund-receipt` | The PSD2 receipt MAY be referenced from `Resolution.refundTransaction` as an `offchain://psd2/<receipt-id>` URI.       |
...
3. **Resolve `receiptHash`** — locate the corresponding `SignedReceipt` (server-side database lookup). If not found, reject with `dispute_unknown_receipt`.
...
- **Arbiter compromise**: a compromised arbiter key can falsely issue `denied` resolutions, blocking legitimate disputes. Mitigations: (a) require arbiter key rotation via on-chain registry, (b) provide an escalation path to an `external` arbiter when `facilitator` denials are contested, (c) track arbiter behavior via reputation registries.
...
| Version | Date       | Change                                                                                                                                                                                                                                                              |
|---------|------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1       | 2026-05-29 | Initial spec. Closed enum `reason`, four-verdict `verdict` field, EIP-712 envelope mirroring offer-and-receipt structure. Four arbiter schemes (`facilitator` / `contract` / `external` / `none`). Composability matrix with `auth-capture`, `batch-settlement`, `exact`. |

---

## References

- [Offer and Receipt extension](./extension-offer-and-receipt.md) — provides the receipt envelope this extension binds to
...
- Related x402 work:
  - x402 PR #2494 — `refund-receipt-v1` / `cancellation-receipt-v1` (PSD2 regulatory receipts; complementary substrate)
