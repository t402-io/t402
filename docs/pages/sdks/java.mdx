...
### @RequirePayment Annotation (v1.7.0+)
...
### Route-Based Configuration (v1.7.0+)
...
### Spring WebFlux (Reactive) (v1.7.0+)
...
Implement custom verification logic for advanced use cases:

```java
import io.t402.server.PaymentVerifier;
import io.t402.types.PaymentPayload;
import io.t402.types.PaymentRequirements;
import io.t402.types.VerificationResponse;
...
public class CustomPaymentVerifier implements PaymentVerifier {
    ...
    public VerificationResponse verify(PaymentPayload payload, PaymentRequirements requirements) {
        ...
        // Custom validation before facilitator
        if (isBlacklisted(payload.getFrom())) {
            return VerificationResponse.invalid("Payer is blacklisted");
        }

        // Amount validation
        if (new BigInteger(payload.getAmount()).compareTo(MAX_AMOUNT) > 0) {
            return VerificationResponse.invalid("Amount exceeds maximum");
        }

        // Delegate to facilitator
        VerificationResponse response = facilitator.verify(payload, requirements);

        // Track metrics
        metrics.recordVerification(payload.getFrom(), response.isValid());
...
Register custom verifier with Spring Boot:
...
Customize payment requirements per-request:
...
public class DynamicPaymentProvider implements PaymentRequirementsProvider {
    ...
    public PaymentRequirements getRequirements(HttpServletRequest request) {
        ...
        BigInteger amount = switch (tier) {
            case "premium" -> BigInteger.valueOf(100_000);    // $0.10
            case "enterprise" -> BigInteger.valueOf(10_000);  // $0.01
            default -> BigInteger.valueOf(1_000_000);         // $1.00
        };
...
| `SafeAccount` | Safe smart account integration (v1.7.0+) |
...
| `PimlicoBundler` | Pimlico bundler with gas sponsorship (v1.7.0+) |
| `AlchemyBundler` | Alchemy bundler with simulation (v1.7.0+) |
...
| `RequirePayment` | Annotation for payment protection (v1.7.0+) |
| `RouteConfig` | YAML-based route configuration (v1.7.0+) |
| `PaymentInterceptor` | HandlerInterceptor for annotation-based validation (v1.7.0+) |
| `T402WebMvcConfigurer` | Auto-registers PaymentInterceptor (v1.7.0+) |

### Spring WebFlux (v1.7.0+)
...
| Class | Description |
|-------|-------------|
| `PaymentPayload` | Payment header structure with Builder (v1.7.0+) |
| `PaymentRequirements` | Payment requirements for an endpoint |
| `ResourceInfo` | Resource metadata for v2 protocol (v1.7.0+) |
| `SupportedResponse` | Full /supported endpoint response (v1.7.0+) |
...
The Java SDK works seamlessly with Kotlin, including coroutines support.
...
class ReactiveApiController {
    ...
    suspend fun getData(): DataResponse {
        // Coroutines work naturally with WebFlux
...
### Kotlin DSL for Payment Requirements
...
class AsyncFacilitatorClient(private val client: HttpFacilitatorClient) {
...
class PaymentFilterTest {
    ...
    @Test
    void shouldVerifyPaymentAndContinue() {
        ...
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.addHeader("X-PAYMENT", validPaymentHeader);
        MockHttpServletResponse response = new MockHttpServletResponse();
