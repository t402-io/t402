...
On NEAR Protocol, the exact scheme uses NEP-141 fungible token transfers with function call access keys for gasless execution.
...
On Aptos, the exact scheme uses the Fungible Asset standard (formerly Coin) for USDT transfers.
...
On Tezos, the exact scheme uses the FA2 (TZIP-12) token standard for USDt transfers.
...
On Polkadot, the exact scheme uses the Asset Hub parachain's Assets pallet for USDT transfers.
...
On Stacks (Bitcoin L2), the exact scheme uses SIP-010 fungible token transfers secured by Bitcoin.
...
\begin{itemize}
    \item \textbf{Up-To Scheme}: Metered payments using EIP-2612 permits for usage-based billing
    \item \textbf{Permit2 Integration}: Uniswap's universal approval system for improved gas efficiency
