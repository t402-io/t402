...
import type { TransactionIntent, GasEstimate, PaymasterData, UserOperation } from '@t402/evm'
...
function getTokenAddress(token: 'USDT0' | 'USDC' | Address, chainName: string): Address {
...
export class MultiSigWdkGaslessClient {
  ...
  private async initiateTransaction(
    ...
    const userOpHash = await this.builder.getUserOpHash(userOp, this.publicClient, this.chainId)
   ...
   */
  async signWithOwner(requestId: string, ownerIndex: number, signer: WDKSigner): Promise<void> {
  ...
  async payWithAllSigners(
    ...
    for (const signer of signers) {
      ...
      const ownerIndex = owners.findIndex((o) => o.toLowerCase() === signer.address.toLowerCase())
