...
Use the `useGaslessPayment` hook (React) or composable (Vue) for UI integration:

```tsx
import { useGaslessPayment } from '@t402/react'; // or '@t402/vue'

const { pay, isLoading, txHash, sponsored, error } = useGaslessPayment({
...
See [@t402/react](/reference/react#usegasledsspayment) and [@t402/vue](/reference/vue#usegasledsspayment) for full API reference.
