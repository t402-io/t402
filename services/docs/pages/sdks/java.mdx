...
### @RequirePayment Annotation (v1.8.0+)
...
### Route-Based Configuration (v1.8.0+)
...
### Spring WebFlux (Reactive) (v1.8.0+)
...
| `SafeAccount` | Safe smart account integration (v1.8.0+) |
...
| `PimlicoBundler` | Pimlico bundler with gas sponsorship (v1.8.0+) |
| `AlchemyBundler` | Alchemy bundler with simulation (v1.8.0+) |
...
| `RequirePayment` | Annotation for payment protection (v1.8.0+) |
| `RouteConfig` | YAML-based route configuration (v1.8.0+) |
| `PaymentInterceptor` | HandlerInterceptor for annotation-based validation (v1.8.0+) |
| `T402WebMvcConfigurer` | Auto-registers PaymentInterceptor (v1.8.0+) |

### Spring WebFlux (v1.8.0+)
...
| Class | Description |
|-------|-------------|
| `PaymentPayload` | Payment header structure with Builder (v1.8.0+) |
| `PaymentRequirements` | Payment requirements for an endpoint |
| `ResourceInfo` | Resource metadata for v2 protocol (v1.8.0+) |
| `SupportedResponse` | Full /supported endpoint response (v1.8.0+) |
