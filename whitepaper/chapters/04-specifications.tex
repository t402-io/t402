...
For a complete version history and migration guide, see \cref{tab:versions} in \cref{sec:versioning}.
...
\code{near} & Network name & NEAR Protocol networks \\
\code{aptos} & Chain ID & Aptos networks \\
\code{tezos} & Network hash & Tezos networks \\
\code{polkadot} & Genesis hash (partial) & Polkadot parachains \\
\code{stacks} & Chain ID & Stacks (Bitcoin L2) networks \\
\code{cosmos} & Chain ID string & Cosmos SDK chains \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htbp]
\centering
\caption{Supported Networks (Partial List)}
...
\midrule
\multicolumn{3}{l}{\textit{EVM Networks (19+)}} \\
...
\midrule
\multicolumn{3}{l}{\textit{Non-EVM Networks}} \\
Solana & \network{solana:5eykt...Kvdp} & USDT \\
TON Mainnet & \network{ton:mainnet} & USDT \\
TRON Mainnet & \network{tron:mainnet} & USDT \\
NEAR Mainnet & \network{near:mainnet} & USDT \\
Aptos Mainnet & \network{aptos:1} & USDT \\
Tezos Mainnet & \network{tezos:NetXdQprcVkpaWU} & USDt \\
Polkadot Asset Hub & \network{polkadot:68d56f15...} & USDT \\
Stacks Mainnet & \network{stacks:1} & USDT \\
Cosmos/Noble & \network{cosmos:noble-1} & USDT \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\code{PAYMENT-SIGNATURE} & Base64-encoded PaymentPayload JSON \\
\code{X-PAYMENT} & V1 alias for PAYMENT-SIGNATURE \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Header} & \textbf{Description} \\
\midrule
\code{PAYMENT-REQUIRED} & Base64-encoded PaymentRequired JSON \\
\code{PAYMENT-RESPONSE} & Base64-encoded SettlementResponse JSON \\
\bottomrule
\end{tabular}
\end{table}
...
Complete JSON Schema~\cite{jsonschema} definitions are provided in Appendix~\ref{app:schemas}. Key validation rules:
