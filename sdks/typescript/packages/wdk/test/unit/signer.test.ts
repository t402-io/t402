...
describe('WDKSigner', () => {
  describe('Constructor validation', () => {
    it('should throw if WDK instance is null', () => {
      ...
      expect(() => new WDKSigner(null as unknown as WDKInstance, 'arbitrum')).toThrow(
        'WDK instance is required',
      )
    })

    it('should throw if WDK instance is undefined', () => {
      expect(() => new WDKSigner(undefined as unknown as WDKInstance, 'arbitrum')).toThrow(
        SignerError,
      )
    })
    ...
    it('should throw if account index is negative', () => {
      ...
      expect(() => new WDKSigner(wdk, 'arbitrum', -1)).toThrow(
  ...
  describe('initialize', () => {
    ...
    it('should throw on getAccount failure', async () => {
      const wdk = createMockWDK()
      ;(wdk.getAccount as ReturnType<typeof vi.fn>).mockRejectedValue(
  ...
  describe('signTypedData', () => {
    ...
    it('should throw if message is null', async () => {
      await expect(
        signer.signTypedData(null as unknown as Parameters<typeof signer.signTypedData>[0]),
  ...
  describe('signMessage', () => {
    ...
    it('should throw if message is null', async () => {
      ...
      await expect(signer.signMessage(null as unknown as string)).rejects.toThrow(
        'Message is required',
      )
    })
    ...
    it('should throw if message is invalid type', async () => {
      ...
      await expect(signer.signMessage(123 as unknown as string)).rejects.toThrow(
  ...
  describe('getTokenBalance', () => {
    ...
    it('should return token balance', async () => {
      ...
      expect(mockAccount.getTokenBalance).toHaveBeenCalledWith(
    ...
    it('should throw on balance fetch failure', async () => {
      ;(mockAccount.getTokenBalance as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error('RPC error'),
      )
      await expect(
        signer.getTokenBalance('0xFd086bC7CD5C481DCC9C85ebE478A1C0b69FCbb9'),
  ...
  describe('estimateGas', () => {
    ...
    it('should throw on invalid to address', async () => {
      await expect(signer.estimateGas({ to: 'invalid' as `0x${string}` })).rejects.toThrow(
        TransactionError,
      )
      await expect(signer.estimateGas({ to: 'invalid' as `0x${string}` })).rejects.toThrow(
        "Invalid 'to' address",
    ...
    it('should throw on empty to address', async () => {
      await expect(signer.estimateGas({ to: '' as `0x${string}` })).rejects.toThrow(
        TransactionError,
      )
    })

    it('should throw on estimation failure', async () => {
      ;(mockAccount.estimateGas as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error('Estimation failed'),
      )
      await expect(
        signer.estimateGas({ to: '0x1234567890123456789012345678901234567890' }),
  ...
  describe('sendTransaction', () => {
    ...
    it('should throw on invalid to address', async () => {
      await expect(signer.sendTransaction({ to: 'invalid' as `0x${string}` })).rejects.toThrow(
        TransactionError,
      )
    })

    it('should throw on empty to address', async () => {
      await expect(signer.sendTransaction({ to: '' as `0x${string}` })).rejects.toThrow(
        TransactionError,
      )
    })
    ...
    it('should throw on invalid hash format', async () => {
      ...
      await expect(
        signer.sendTransaction({ to: '0x1234567890123456789012345678901234567890' }),
      ).rejects.toThrow(TransactionError)
      await expect(
        signer.sendTransaction({ to: '0x1234567890123456789012345678901234567890' }),
    ...
    it('should throw on transaction failure', async () => {
      ;(mockAccount.sendTransaction as ReturnType<typeof vi.fn>).mockRejectedValue(
        new Error('Tx failed'),
      )
      await expect(
        signer.sendTransaction({ to: '0x1234567890123456789012345678901234567890' }),
