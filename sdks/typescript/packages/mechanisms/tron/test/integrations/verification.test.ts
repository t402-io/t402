...
function createMockSigner(...addresses: string[]): FacilitatorTronSigner {
  return {
    ...
    waitForTransaction: vi
      .fn()
      .mockResolvedValue({ success: true, txId: 'mock-tx-id' } as TransactionConfirmation),
...
describe('TRON Verification Edge Cases', () => {
  ...
  describe('Facilitator Configuration', () => {
    ...
    it('should support multiple addresses', () => {
      const mockSigner = createMockSigner(VALID_FACILITATOR, VALID_RECIPIENT, VALID_SENDER)
      const facilitator = new ExactTronFacilitator(mockSigner)
  ...
  describe('Payload Structure', () => {
    ...
    it('should have authorization with required fields', () => {
      const payload = createMockPayload()
      const authorization = (payload.payload as Record<string, unknown>).authorization as Record<
        string,
        unknown
      >
  ...
  describe('Settlement', () => {
    ...
    it('should handle broadcast failure', async () => {
      const mockSigner = createMockSigner(VALID_FACILITATOR)
      ;(mockSigner.broadcastTransaction as ReturnType<typeof vi.fn>).mockRejectedValue(
