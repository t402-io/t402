...
Use the `useMultiSigPayment` hook (React) or composable (Vue) for UI integration:

```tsx
import { useMultiSigPayment } from '@t402/react'; // or '@t402/vue'

const { initiate, submit, status, isReady, threshold, collectedCount } = useMultiSigPayment({
...
See [@t402/react](/reference/react#usemultisigpayment) and [@t402/vue](/reference/vue#usemultisigpayment) for full API reference.
