...
\tprotocol{} is \textbf{trust-minimized}, not trustless. The key trust assumptions are:

\begin{itemize}
    \item \textbf{Facilitator as settlement agent}: The Facilitator submits signed transactions on behalf of the payer. It cannot redirect funds (the \field{payTo} address is bound in the payer's signature), but it can \emph{refuse to settle} or \emph{delay settlement}. This is a liveness dependency, not a safety risk.

    \item \textbf{Facilitator has no custody}: The payer's signature authorizes a specific transfer. The Facilitator submits it but never holds user funds. If compromised, a Facilitator cannot extract funds beyond the amounts already authorized by payers.

    \item \textbf{On-chain verification}: All settlement transactions are verified by the blockchain's token contract. The Facilitator cannot forge or modify the authorized transfer parameters.

    \item \textbf{Current limitations}: There is no on-chain mechanism to cap total Facilitator settlement volume or enforce multi-sig for high-value transactions. These are areas for future protocol hardening (see Chapter~\ref{ch:future-work}).
\end{itemize}
...
EIP-3009~\cite{eip3009} uses ECDSA signatures over the secp256k1 curve. The security reduces to:
...
TRON & $\sim$3 seconds & Low \\
NEAR & $\sim$1.2 seconds & Low \\
Aptos & $\sim$1 second & Very Low \\
Tezos & $\sim$30 seconds & Low \\
Polkadot & $\sim$12 seconds & Low \\
Stacks & $\sim$10 minutes & Moderate \\
Cosmos/Noble & $\sim$6 seconds & Low \\
...
\begin{proof}
The EIP-712~\cite{eip712} typed data includes the \field{to} parameter in the hash:
