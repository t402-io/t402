...
This specification defines the core t402 protocol for internet-native payments. It covers:
...
t402 is made up of three core components:
...
This specification is based on the t402 protocol implementation and documentation available in the [T402 repository](https://github.com/t402-io/t402). It aims to provide a comprehensive and implementation-agnostic specification for the t402 protocol.

**2. Core Payment Flow**

The t402 protocol follows a standard request-response cycle with payment integration:
...
The t402 protocol involves three primary components:
...
The t402 protocol defines standard response types with specific semantics:
...
| ------------- | -------- | -------- | ---------------------------------------------------------------------- |
| `t402Version` | `number` | Required | Protocol version identifier (must be 2)                                |
...
This section describes the payment schemes supported by the t402 protocol. Payment schemes define how payments are formed, validated, and settled on specific payment networks. Schemes are independent of the underlying transport mechanism.
...
The facilitator provides HTTP REST APIs for payment verification and settlement. This allows resource servers to delegate blockchain operations to trusted third parties or host the endpoints themselves. Note that while the core t402 protocol is transport-agnostic, facilitator APIs are currently standardized as HTTP endpoints.
...
| Field Name    | Type     | Required | Description                                                |
| ------------- | -------- | -------- | ---------------------------------------------------------- |
| `t402Version` | `number` | Required | Protocol version supported (2 for v2)                      |
| `scheme`      | `string` | Required | Payment scheme identifier (e.g., "exact")                  |
| `network`     | `string` | Required | Blockchain network identifier in CAIP-2 format             |
...
The t402 protocol includes a discovery mechanism that allows clients to find and explore available t402-enabled resources. This enables the creation of marketplaces (known as "Bazaars") where users can discover and access monetized APIs and digital services.

Discovery is currently implemented as HTTP REST APIs, though the discovered resources may use any t402-supported transport.

8.1 GET /discovery/resources

List discoverable t402 resources from the Bazaar.
...
| `t402Version` | `number` | Required | Protocol version supported by the resource                      |
...
The Bazaar is a marketplace ecosystem where t402-enabled resources can be discovered and accessed. Key features:
...
The t402 protocol defines standard error codes that may be returned by facilitators or resource servers. These error codes help clients understand why a payment failed and take appropriate action.
...
- **`invalid_t402_version`**: Protocol version is not supported
...
The t402 protocol implements multiple layers of protection against replay attacks:
...
Networks in t402 v2 use CAIP-2 (Chain Agnostic Improvement Proposal) format: `namespace:reference`.
...
The t402 protocol enables diverse monetization scenarios across the internet. While the core protocol is HTTP-native and chain-agnostic, specific implementations can vary based on use case requirements.

### 12.1 AI Agent Integration

AI agents can use t402 to autonomously pay for resources and services. The protocol supports:
...
Applications can implement t402 for:
...
t402 integrates across multiple transport layers:
...
- **Custom Protocols**: Any request-response based system can implement t402 payment flows

### 12.4 Server Frameworks

t402 integrates with popular frameworks:
...
Clients across different transports can be enhanced with t402 payment capabilities:
...
| ------- | ----------- | ----------------------------------------------------------------- | ------------------------- |
| v2.0    | 2025-12-9   | Protocol v2: CAIP-2 networks, restructured PaymentPayload/Required, ResourceInfo separation, extensions support | t402 team |
