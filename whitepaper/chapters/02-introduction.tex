...
\tprotocol{} focuses exclusively on stablecoin payments (primarily USDT~\cite{usdt} and USDT0 via LayerZero~\cite{layerzero}) for several reasons:
...
The advent of blockchain technology, stablecoins~\cite{usdt}, and standardized signing schemes (EIP-712~\cite{eip712}, EIP-3009~\cite{eip3009}) has finally made the vision practical.
...
\begin{tabular}{l l l}
\toprule
\textbf{Network Family} & \textbf{Mechanism} & \textbf{Signature} \\
\midrule
...
\begin{description}
    ...
    \item[Chapter 5: Payment Schemes] describes payment schemes, focusing on the ``exact'' scheme across all supported chains and the ``up-to'' scheme for metered payments.
