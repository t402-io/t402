...
\begin{enumerate}
    ...
    \item \textbf{Trust Minimization}: No single party can unilaterally redirect funds or forge payments. The Facilitator acts as a settlement agent but never has custody of user funds. The blockchain serves as the ultimate source of truth for all transfers.
...
\node[subcomponent, draw=orange, fill=orange!5] at ([xshift=-3.5cm]blockchain.center) (evm) {EVM};
\node[subcomponent, draw=orange, fill=orange!5] at ([xshift=-1.8cm]blockchain.center) (solana) {Solana};
\node[subcomponent, draw=orange, fill=orange!5] at ([xshift=0cm]blockchain.center) (ton) {TON};
\node[subcomponent, draw=orange, fill=orange!5] at ([xshift=1.8cm]blockchain.center) (tron) {TRON};
\node[subcomponent, draw=orange, fill=orange!5] at ([xshift=3.5cm]blockchain.center) (more) {+6 more};
...
\begin{description}
    ...
    \item[Gas Sponsorship] Cover transaction fees for gasless payment flows (EIP-3009~\cite{eip3009}, ERC-4337~\cite{eip4337})
\end{description}
...
% Top row chains (5)
\node[chainbox, draw=blue!70, fill=blue!15] (evm) at (-5.5,0) {EVM (19+)};
\node[chainbox, draw=violet!70, fill=violet!15] (sol) at (-2.75,0) {Solana};
\node[chainbox, draw=cyan!70, fill=cyan!15] (ton) at (0,0) {TON};
\node[chainbox, draw=red!70, fill=red!15] (tron) at (2.75,0) {TRON};
\node[chainbox, draw=green!70, fill=green!15] (near) at (5.5,0) {NEAR};

% Bottom row chains (5)
\node[chainbox, draw=orange!70, fill=orange!15] (aptos) at (-5.5,-1.5) {Aptos};
\node[chainbox, draw=teal!70, fill=teal!15] (tezos) at (-2.75,-1.5) {Tezos};
\node[chainbox, draw=pink!70, fill=pink!15] (polkadot) at (0,-1.5) {Polkadot};
\node[chainbox, draw=brown!70, fill=brown!15] (stacks) at (2.75,-1.5) {Stacks};
\node[chainbox, draw=olive!70, fill=olive!15] (cosmos) at (5.5,-1.5) {Cosmos};

% Features (top row)
\node[font=\tiny] at (-5.5,-0.6) {EIP-3009};
\node[font=\tiny] at (-2.75,-0.6) {SPL Token};
\node[font=\tiny] at (0,-0.6) {Jetton};
\node[font=\tiny] at (2.75,-0.6) {TRC-20};
\node[font=\tiny] at (5.5,-0.6) {NEP-141};

% Features (bottom row)
\node[font=\tiny] at (-5.5,-2.1) {FA};
\node[font=\tiny] at (-2.75,-2.1) {FA2};
\node[font=\tiny] at (0,-2.1) {Asset Hub};
\node[font=\tiny] at (2.75,-2.1) {SIP-010};
\node[font=\tiny] at (5.5,-2.1) {Native};

% Connections - top row
\draw[->, thick, violet] (t402.south) ++(-4.2,0) -- (evm.north);
\draw[->, thick, violet] (t402.south) ++(-2.1,0) -- (sol.north);
\draw[->, thick, violet] (t402.south) ++(0,0) -- (ton.north);
\draw[->, thick, violet] (t402.south) ++(2.1,0) -- (tron.north);
\draw[->, thick, violet] (t402.south) ++(4.2,0) -- (near.north);

% Connections - bottom row (through top row)
\draw[->, thick, violet!60] (evm.south) -- (aptos.north);
\draw[->, thick, violet!60] (sol.south) -- (tezos.north);
\draw[->, thick, violet!60] (ton.south) -- (polkadot.north);
\draw[->, thick, violet!60] (tron.south) -- (stacks.north);
\draw[->, thick, violet!60] (near.south) -- (cosmos.north);

\end{tikzpicture}
\caption{T402 multi-chain architecture: 10 blockchain families, 44+ networks}
\label{fig:multichain}
\end{figure}

Each blockchain network is identified using CAIP-2 format~\cite{caip2} (e.g., \code{eip155:8453} for Base), enabling:
