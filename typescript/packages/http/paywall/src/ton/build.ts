...
const OUTPUT_TS = path.join("src/ton/gen", "template.ts");
...
const OUTPUT_PY = path.join(PYTHON_DIR, "ton_paywall_template.py");
const OUTPUT_GO = path.join(GO_DIR, "ton_paywall_template.go");
...
/**
 * Builds the TON paywall HTML template with bundled JS and CSS.
...
async function build() {
  try {
    ...
    console.log("[TON] Build completed successfully!");
...
/**
 * The pre-built TON paywall template with inlined CSS and JS
 */
export const TON_PAYWALL_TEMPLATE = ${JSON.stringify(html)};
`;

      // Generate Python template file
      const pyContent = `# THIS FILE IS AUTO-GENERATED - DO NOT EDIT
TON_PAYWALL_TEMPLATE = ${JSON.stringify(html)}
`;
...
// TONPaywallTemplate is the pre-built TON paywall template with inlined CSS and JS
const TONPaywallTemplate = ${JSON.stringify(html)}
`;

      fs.writeFileSync(OUTPUT_TS, tsContent);
      console.log(`[TON] Generated template.ts (${(html.length / 1024 / 1024).toFixed(2)} MB)`);

      // Write the Python template file
      if (fs.existsSync(PYTHON_DIR)) {
        fs.writeFileSync(OUTPUT_PY, pyContent);
        console.log(
          `[TON] Generated Python ton_paywall_template.py (${(html.length / 1024 / 1024).toFixed(2)} MB)`,
        );
      } else {
        console.warn(`[TON] Python directory not found: ${PYTHON_DIR}`);
      }
      ...
      if (fs.existsSync(GO_DIR)) {
        ...
        console.log(
          `[TON] Generated Go ton_paywall_template.go (${(html.length / 1024 / 1024).toFixed(2)} MB)`,
        );
      } else {
        console.warn(`[TON] Go directory not found: ${GO_DIR}`);
      }
    } else {
