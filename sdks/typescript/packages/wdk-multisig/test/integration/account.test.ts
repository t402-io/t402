...
import { createMockWDKSigners, createMockPublicClient, TEST_ADDRESSES } from './mocks.js'
...
describe('MultiSigWdkSmartAccount Integration', () => {
  ...
  describe('Init Code Generation', () => {
    it('should generate init code for undeployed account', async () => {
      ...
      expect(
        initCode.toLowerCase().includes(SAFE_4337_ADDRESSES.proxyFactory.toLowerCase().slice(2)),
      ).toBe(true)
    })
  ...
  describe('Execute Encoding', () => {
    it('should encode single execute call', async () => {
      ...
      const callData = account.encodeExecute(TEST_ADDRESSES.recipient, 0n, '0x' as Hex)
