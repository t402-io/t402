...
describe('BalanceCache', () => {
  ...
  describe('Token balance caching', () => {
    ...
    it('should get or fetch token balance', async () => {
      ...
      const result1 = await cache.getOrFetchTokenBalance('arbitrum', '0xUSDT', '0x1234', fetcher)
      expect(result1).toBe(2000000n)
      expect(fetcher).toHaveBeenCalledTimes(1)

      const result2 = await cache.getOrFetchTokenBalance('arbitrum', '0xUSDT', '0x1234', fetcher)
