...
The Java SDK provides payment filters for 5 HTTP frameworks: Jakarta Servlet, Spring MVC, Spring WebFlux, Micronaut, and Quarkus. Each filter implements the full T402 v2 payment lifecycle with V2-first header detection and V1 fallback.
...
| Quarkus | `io.t402.quarkus` | `PaymentFilter` | Synchronous (JAX-RS) |
...
The Quarkus filter implements JAX-RS `ContainerRequestFilter` and `ContainerResponseFilter` for the full payment lifecycle.
