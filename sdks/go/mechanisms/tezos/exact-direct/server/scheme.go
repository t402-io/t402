...
// ExactDirectTezosServerConfig holds optional configuration for the server.
...
// ExactDirectTezosScheme implements the SchemeNetworkServer interface for Tezos
// exact-direct payments (V2). It handles price parsing and enhancement of payment
...
// NewExactDirectTezosScheme creates a new ExactDirectTezosScheme server.
//
// Args:
//
...
func NewExactDirectTezosScheme(config ...*ExactDirectTezosServerConfig) *ExactDirectTezosScheme {
	cfg := ExactDirectTezosServerConfig{}
...
func (s *ExactDirectTezosScheme) Scheme() string {
...
func (s *ExactDirectTezosScheme) RegisterMoneyParser(parser t402.MoneyParser) *ExactDirectTezosScheme {
...
func (s *ExactDirectTezosScheme) ParsePrice(price t402.Price, network t402.Network) (t402.AssetAmount, error) {
	networkStr := string(network)

	// Validate network
	if !tezos.IsTezosNetwork(networkStr) {
...
func (s *ExactDirectTezosScheme) EnhancePaymentRequirements(
	ctx context.Context,
	...
	// Get network config
	networkConfig, ok := tezos.GetNetworkConfig(networkStr)
	if !ok {
	...
	if requirements.Asset == "" {
		requirements.Asset = tezos.CreateAssetIdentifier(networkStr, token.ContractAddress, token.TokenID)
	}
	...
	// Add token metadata to extra (only if not already set)
	...
	if supportedKind.Extra != nil {
		...
		// Copy standard fields from supportedKind (overrides defaults)
...
func (s *ExactDirectTezosScheme) parseMoneyToDecimal(price t402.Price) (float64, error) {
	switch v := price.(type) {
	case string:
...
func (s *ExactDirectTezosScheme) defaultMoneyConversion(amount float64, network t402.Network) (t402.AssetAmount, error) {
	networkStr := string(network)
	...
	return t402.AssetAmount{
		Amount: atomicAmount,
		Asset:  tezos.CreateAssetIdentifier(networkStr, token.ContractAddress, token.TokenID),
...
func (s *ExactDirectTezosScheme) getDefaultToken(network string) *tezos.TokenInfo {
	// If a preferred token is configured, try to use it
	if s.config.PreferredToken != "" {
		token, ok := tezos.GetTokenInfo(network, s.config.PreferredToken)
		if ok {
			return &token
		}
	}
	...
	config, ok := tezos.GetNetworkConfig(network)
	if !ok {
		return nil
	}
