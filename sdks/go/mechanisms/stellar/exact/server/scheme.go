...
func (s *ExactStellarScheme) RegisterMoneyParser(parser t402.MoneyParser) *ExactStellarScheme {
...
func (s *ExactStellarScheme) ParsePrice(price t402.Price, network t402.Network) (t402.AssetAmount, error) {
...
func (s *ExactStellarScheme) parseMoneyToDecimal(price t402.Price) (float64, error) {
...
func (s *ExactStellarScheme) defaultMoneyConversion(amount float64, config *stellar.NetworkConfig) (t402.AssetAmount, error) {
...
func (s *ExactStellarScheme) EnhancePaymentRequirements(
	...
	// Get asset info
	var assetInfo *stellar.AssetInfo
	if requirements.Asset != "" {
		assetInfo, err = stellar.GetAssetInfo(networkStr, requirements.Asset)
		if err != nil {
			return requirements, err
		}
	} else {
		assetInfo = &config.DefaultAsset
		requirements.Asset = assetInfo.ContractAddress
	}

	// Ensure amount is in the correct format (smallest unit)
	if requirements.Amount != "" && strings.Contains(requirements.Amount, ".") {
		amount, err := stellar.ParseAmount(requirements.Amount, assetInfo.Decimals)
		if err != nil {
