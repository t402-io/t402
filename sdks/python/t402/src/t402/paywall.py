...
def get_paywall_template(network: str, delivery_mode: str = "cdn") -> str:
    ...
    Args:
        ...
        delivery_mode: Reserved for future use. Currently only "cdn"
            (the lightweight shell that loads UI assets via CDN) is
            shipped. The previous "inline" mode (full embedded HTML,
            ~5MB per response) was removed in v3.0 — it predated the
            CDN-hosted UI and the size made it impractical. Pass
            anything; behavior is always CDN-mode now.
    """
