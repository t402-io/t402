 ...
 *
 * GET  /                          — Dashboard UI (empty state or data view)
...
app.use((_req, res, next) => {
  ...
  res.set(
    ...
    "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src 'self'; form-action 'self'; frame-ancestors 'none'; base-uri 'none'",
  );
...
// ── CORS — intentionally permissive (*) for public demo dashboard ───
app.use((_req, res, next) => {
 ...
 *           budget: object|null, stats: object|null, payments: Array, alerts: Array }} data
...
(function() {
  ...
  function startAutoRefresh() {
    refreshTimer = setInterval(function() { refreshData(); }, 30000);
  }
