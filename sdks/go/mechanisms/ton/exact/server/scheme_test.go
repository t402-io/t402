...
func TestParsePrice_AssetAmountMap(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantAmount: "1000000",
			wantAsset:  ton.USDTMainnetAddress,
		},
		{
		...
		},
		{
			...
			wantAmount: "3000000",
			wantAsset:  ton.USDTTestnetAddress,
		},
	}
...
func TestParsePrice_NumberInput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			scheme := NewExactTonScheme()
			result, err := scheme.ParsePrice(tt.price, ton.TonMainnetCAIP2)
			if err != nil {
				t.Fatalf("ParsePrice() error: %v", err)
			}
			if result.Amount != tt.wantAmount {
...
func TestDefaultMoneyConversion_IncludesTokenMetadata(t *testing.T) {
	...
	result, err := scheme.ParsePrice(1.00, ton.TonMainnetCAIP2)
	if err != nil {
		t.Fatalf("ParsePrice() error: %v", err)
	}
	...
	if result.Extra["decimals"] != ton.DefaultDecimals {
		t.Errorf("Extra.decimals = %v, want %d", result.Extra["decimals"], ton.DefaultDecimals)
	}
}
